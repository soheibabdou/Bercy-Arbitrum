...
async function verifyPayment(txHash) {
  try {
    ...
    const wallet = process.env.ARBITRUM_USDC_WALLET.toLowerCase();
...
async function getLiveRates() {
  try {
    const [ecbRes, cgRes] = await Promise.all([
      ...
      fetch("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,arbitrum-2,binancecoin,matic-network,chainlink,near,cosmos,ripple,algorand,celo&vs_currencies=usd"),
    ]);
    ...
    const crypto = {
      ...
      SOL: cg["solana"]?.usd, AVAX: cg["arbitrum-2"]?.usd,
...
<html><head><title>Bercy Arbitrum Pay</title>
<style>
  *{box-sizing:border-box}
  body{background:#0a0a0a;color:#213BFF;font-family:monospace;margin:0;padding:40px 20px;display:flex;justify-content:center}
  .wrap{max-width:700px;width:100%}
  h1{color:#FFFFFF;margin-top:0}
  a{color:#213BFF}
  hr{border:none;border-top:1px solid #222;margin:20px 0}
  .badge{border:1px solid #213BFF;padding:4px 10px;margin:4px;display:inline-block;border-radius:4px;font-size:13px;color:#213BFF}
  .ep{background:#111;padding:10px 14px;margin:6px 0;border-radius:4px;display:block;color:#213BFF}
  .ep:hover{background:#1a1a1a}
  .ep a{color:#FFFFFF;text-decoration:none}
</style>
</head><body><div class="wrap">
<h1>🔺 Bercy Arbitrum Pay</h1>
<p>World's first <b>x402 + AC2</b> cross-border payment layer on Arbitrum One Mainnet.</p>
<div>
  <span class="badge">⚡ x402 on Arbitrum</span>
...
<hr>
<p>🔺 Arbitrum One | Chain 42161 | x402 + AC2<br>
...
app.get("/api/health", async (req, res) => {
  try {
    ...
    res.json({ status: "ok", service: "Bercy Arbitrum Pay", network: "Arbitrum One Mainnet", protocols: ["x402", "AC2"], totalCorridors: 95 });
  }
});
...
app.post("/api/authorize", async (req, res) => {
  ...
  const approval_id = "ac2_arb_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);
  res.json({
    approved: true,
    approval_id,
    from, to, amount,
    ...
    message: "AC2: Human approval granted. Proceed with bercy_pay_arbitrum.",
    timestamp: new Date().toISOString()
  });
});
...
app.post("/api/orchestrate", async (req, res) => {
  ...
  if (!payment) return res.status(402).json({
    ...
    pay_to: process.env.ARBITRUM_USDC_WALLET,
    message: "Send $0.10 USDC on Arbitrum One, include tx hash as X-PAYMENT header"
  });
  ...
  const tag = "arb_" + Date.now().toString(36);
  res.json({
    success: true,
...
app.listen(PORT, () => console.log("Bercy Arbitrum Pay running on port " + PORT));
